...
export default function AdminCustomers() {
  ...
  const fetchCustomers = async () => {
    try {
      ...
      const { data } = await axios.get('https://saasecommerce.vercel.app/api/customers', config);
      setCustomers(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const fetchCustomerDetails = async (id) => {
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const { data } = await axios.get(`https://saasecommerce.vercel.app/api/customers/${id}`, config);
  ...
  const toggleStatus = async (id) => {
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      await axios.put(`https://saasecommerce.vercel.app/api/customers/${id}/status`, {}, config);
      fetchCustomers();
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      await axios.delete(`https://saasecommerce.vercel.app/api/customers/${id}`, config);
      fetchCustomers();
      if (selectedCustomer && selectedCustomer._id === id) {
