...
export default function AdminReviews() {
  ...
  const fetchReviews = async () => {
    try {
      ...
      const { data } = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/reviews`, config);
      setReviews(data);
    } catch (err) {
      console.error(err);
    } finally {
  ...
  const handleUpdateStatus = async (productId, reviewId, status) => {
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      await axios.put(`${import.meta.env.VITE_BACKEND_URL}/api/reviews/${productId}/${reviewId}/status`, { status }, config);
      fetchReviews();
  ...
  const handleDelete = async (productId, reviewId) => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      await axios.delete(`${import.meta.env.VITE_BACKEND_URL}/api/reviews/${productId}/${reviewId}`, config);
      fetchReviews();
